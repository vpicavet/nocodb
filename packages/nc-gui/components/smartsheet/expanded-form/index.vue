...
<template>
    ...
    <div class="h-[85vh] xs:(max-h-full) max-h-215 flex flex-col p-6">
      <div class="flex h-9.5 flex-shrink-0 w-full items-center nc-expanded-form-header relative mb-4 justify-between">
        <template v-if="!isMobileMode">
          ...
          <div class="flex gap-2">
            <NcButton v-if="!isNew && rowId" :disabled="isLoading" type="secondary" @click="copyRecordUrl()">
  ...
  <GeneralDeleteModal v-model:visible="showDeleteRowModal" :on-delete="() => onConfirmDeleteRowClick()" entity-name="Record">
  ...
  <NcModal v-model:visible="preventModalStatus" size="small">
    <div class="">
      ...
      <div class="flex flex-row justify-end gap-x-2 mt-5">
        ...
        <NcButton key="submit" label="Rename Table" loading-label="Renaming Table" type="primary" @click="saveChanges">
          {{ $t('tooltip.saveChanges') }}
        </NcButton>
      </div>
    </div>
